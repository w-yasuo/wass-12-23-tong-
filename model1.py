...
class SqueezeNet(nn.Module):
    def __init__(self, args, num_classes: int = 3, dropout: float = 0.5, ) -> None:
        super().__init__()
        self.args = args
        ...
        self._fc1 = nn.Linear(725355 * (self.args.batch_size), 256)
        self._fc2 = nn.Linear(725355 * (self.args.batch_size), 256)
        self._fc3 = nn.Linear(725355 * (self.args.batch_size), 256)
    ...
    def extract_features(self, input):
        ...
        # print(input_pooling.shape)  # torch.Size([128, 3, 27, 27])
        # print(conv1_pooling.shape)  # torch.Size([128, 96, 27, 27])
        # print(fire3_pooling.shape)  # torch.Size([128, 128, 27, 27])
        # print(fire5_pooling.shape)  # torch.Size([128, 256, 27, 27])
        # print(conv10_deconv.shape)  # torch.Size([128, 512, 27, 27])
        ...
        # print(fina_x.shape)
        # x = fina_x.reshape(self.args.batch_size, 725355)
        x = torch.tensor(fina_x)
        # print("x.shape:", x.shape)  # torch.Size([self.args.batch_size, 725355])
        fc1 = self._fc1(x)
        fc2 = self._fc2(x)
        fc3 = self._fc3(x)
        return fc1, fc2, fc3
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("x1.shape:", x1.shape)  # x1.shape: torch.Size([1, 256])
        ...
        # pred = pred.reshape((self.args.batch_size, 256, 1, 3))
